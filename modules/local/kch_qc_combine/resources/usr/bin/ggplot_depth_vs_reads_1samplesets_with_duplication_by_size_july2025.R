...
dataset1 <- args[1]
name1 <- args[2]
assay_and_sampleskey <- args[3]
spanin <- args[4]
depth <- args[5]
reads_required <- args[6]
...
print(title)
output<-paste0(assay_and_sampleskey,"_","depth vs reads",depthfield,"_ggplot.pdf")
...
gg1<-ggplot(data=df1, aes(obs_Total_processed_paired_reads, get(depthfield1))) + 
    ...
    scale_colour_manual(name="Data key:", values=c("red")) + 
